...
class SocketCommunication
{
    ...
    static void* communicateWithServer(void* args)
    {
        ...
        const string curr_dir="student_documents/";
        DIR* dir;
        struct dirent* entry;
        ...
        do
        {
            ...
            if(file_name.size()>4 && file_name.substr(file_name.size()-4)==".pdf")
            {
                ...
                memset(buffer,0,sizeof(buffer));
                string fileInfo="SEND FILE INFO";
                send(socketFileDescriptor,fileInfo.c_str(),fileInfo.size(),0);

                //may need to add another recv -->added
                char verificationMessage[256]={0};
                recv(socketFileDescriptor,verificationMessage,sizeof(verificationMessage), 0);
                cout << "Server Response: " << verificationMessage << endl;
                fileReader.close();
                if(docsEnded)
                break;
            }
